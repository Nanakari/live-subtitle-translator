...
function settings() { return { displayMode: displayMode.value, subtitleLanguage: subtitleLanguage.value, overlayScope: overlayScope.value }; }
...
Promise.all([chrome.storage.local.get("subtitleSettings"), chrome.storage.session.get("lastPluginError")]).then(([{ subtitleSettings }, { lastPluginError }]) => { if (subtitleSettings) { overlayScope.value = subtitleSettings.overlayScope || "captured"; displayMode.value = subtitleSettings.displayMode || "sentence"; subtitleLanguage.value = subtitleSettings.subtitleLanguage || "bilingual"; } if (lastPluginError) status.textContent = lastPluginError; });
overlayScope.addEventListener("change", saveSettings);
